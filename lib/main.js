...
export default class Main {
  ...
  constructor(GL){
    ...
    // this.camera = new Camera([0,20,1],[0,0,-5],[0,1,0]);
    this.camera = new Camera([2,10,35],[0,0,5],[0,1,0]);
    ...
    let box = new NewCube(GL, this.cubeShader);

    // cube.translate(0,10,0);
    this.cubeRenderer.addRenderObject(box);

    // this.cubeShaderTransparent = new ShaderProgram(vertexShader, fragmentShader,["color"],["position", "normal"], GL);
    this.cubeShaderTransparent = new ShaderProgram(waterVS, waterFS,["color"],["position", "normal"], GL);
    this.cubeRendererTransparent = new Renderer(GL, this.cubeShaderTransparent);
    let cube = new Cube(GL, this.cubeShaderTransparent);
    this.cubeRendererTransparent.addRenderObject(cube);

    this.waterShader = new ShaderProgram(waterVS, waterFS, ["color", "time", "amplitude", "wavelength", "frequency", "sampler", "sampler2"], ["position" , "texCoord"], GL);
  ...
  renderLoop(){
    ...
    // this.cubeShaderTransparent.start();

    // this.cubeRendererTransparent.render(
    //   this.deltaTime, 
    //   this.totalTime,
    //   viewMatrix, 
    //   this.projectionMatrix);

    // this.cubeShaderTransparent.stop();
  }
